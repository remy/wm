  ...
  <div class="root">
      ...
      <ul v-on:change.capture="handleChange">
        ...
        <li>
          <label>
            ...
            <span>It's complicated, I can make webmention notifications manually</span>
          </label>
        </li>
        <li>
          <label>
    ...
    <div v-show="selected" class="highlight">
      <div v-if="selected === 'feed'">
        <p>Your ideal approach is to use IFTTT to run a recipe when a new item is published to your feed and to trigger a call to webmention.app.</p>
        <p>
          <n-link
            to="/docs#using-ifttt-to-trigger-checks"
          >Read the full walk through can be seen here.</n-link>
        </p>
      </div>
      ...
      <div v-if="selected === 'cli'">
        <p>
          ...
          <code>webmention</code> command line tool. It doesn't rely on this web site at all and can be executed on Windows, Mac and Unix-based platforms.
        </p>
        <p>
          <n-link
            to="/docs#using-the-command-line"
          >Read how to install and use the command line tool.</n-link>
        </p>
      </div>
      ...
      <div v-if="selected === 'complicated'">
        ...
        <p>
          <n-link to="/test">Use the manual test and send tool.</n-link>
        </p>
      </div>

      <div v-if="selected === 'docs'">
        <p>
          Take a peruse of the documentation and see if there's anything that matches. If not, feel free to
          <a href="https://github.com/remy/wm/issues/new" target="_blank">open an issue</a> to see if there's something can be solved for you.
        </p>
        <p>
          <n-link to="/docs">Browse the documentation and recipes.</n-link>
        </p>
      </div>
    </div>
  </div>
